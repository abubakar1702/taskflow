    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <h2 className="text-xl font-semibold text-gray-900 mb-1">Failed to load tasks</h2>
            ...
            <div className="max-w-6xl mx-auto">
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900">
                            In Progress Tasks
                            <span className="ml-2 text-gray-500 font-medium">({tasks?.length || 0})</span>
                        </h1>
                ...
                {!tasks || tasks.length === 0 ? (
                    <div className="bg-white rounded-xl border border-gray-200 shadow-sm py-20 text-center">
                        ...
                        <p className="text-gray-500">Nothing is running right now.</p>
                    </div>
                ) : (
                    <div className="bg-white rounded-xl border border-gray-200 shadow-sm divide-y divide-gray-100">
                        {tasks.map((task) => (
                            <div key={task.id} className="p-4 flex flex-col gap-4 md:flex-row md:items-center md:justify-between hover:bg-gray-50 transition">
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2">
                                        <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                                        <Link to={`/tasks/${task.id}`} className="text-lg font-semibold text-gray-900 hover:text-blue-600 truncate">
                                            {task.title}
                                        </Link>
                                    </div>
                                    <div className="mt-1 text-sm text-gray-500 flex flex-wrap items-center gap-3">
                                        <span className="font-medium text-gray-700">{task.project?.name || "No Project"}</span>
                                            ...
                                            {task.assignees?.length ? (
                                                <div className="flex items-center -space-x-2">
                                                    {task.assignees.slice(0, 3).map((user, index) => (
                                                        <div key={user.id} className="relative ring-2 ring-white rounded-full" style={{ zIndex: 3 - index }}>
                                ...
                                <div className="flex justify-end md:min-w-[150px]">
                                    <div className={`flex items-center gap-2 px-4 py-2 rounded-lg border ${task.timer_start_time ? "bg-blue-50 border-blue-100" : "bg-amber-50 border-amber-100"}`}>
                                        <FaClock className={`w-4 h-4 ${task.timer_start_time ? "text-blue-600 animate-spin-slow" : "text-amber-500"}`} />
                                        <span className={`font-mono text-lg font-bold ${task.timer_start_time ? "text-blue-700" : "text-amber-600"}`}>
